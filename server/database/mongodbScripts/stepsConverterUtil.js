/**
 * This file contains functions that is used to convert and upsert stepsObjects into our Database.
 * Ensure that both the src and dest MongoDB servers are running before executing.
 */
...
module.exports.upsertModule = upsertModule;
...
function upsertGuest(stepsGuestObj, callback) {
  async.waterfall([
    (callback) => {
      // Extract out relevant information from each Guest
          ...
          if (err) {
            ...
            Event.where(eventQuery).lean().findOne((err, event) => {
              if (err) {
                ...
                // Event was not Previously Inserted
                callback(null);
              }
            });
          } else {
            // User was not Previously Inserted
            callback(null);
          }
        });
      } else {
...
module.exports.upsertGuest = upsertGuest;
